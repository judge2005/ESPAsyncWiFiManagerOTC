...
void AsyncWiFiManager::setupConfigPortal() {
  ...
  server->on("/t", std::bind(&AsyncWiFiManager::handleTime, this,std::placeholders::_1)).setFilter(ON_AP_FILTER);
...
void AsyncWiFiManager::handleTime(AsyncWebServerRequest *request) {
  DEBUG_WM(F("Time"));

  _time = request->getParam("time", true, false)->value();

  String page = FPSTR(WFM_HTTP_HEAD);
  page.replace("{v}", "Time Set");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += F("<meta http-equiv=\"refresh\" content=\"5; url=/\">");
  page += FPSTR(HTTP_HEAD_END);
  page += FPSTR(HTTP_TIME_SET);
  page += FPSTR(HTTP_END);

  request->send(200, "text/html", page);

  DEBUG_WM(F("Sent wifi time page"));
}
